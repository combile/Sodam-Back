...
import { recommendationsAPI, ScoreRequest, ScoreResponse } from "../api/recommendations";
import styles from "./MarketScoreCalculator.module.css";

interface MarketScoreCalculatorProps {
  onScoreCalculated?: (score: ScoreResponse) => void;
}

const MarketScoreCalculator: React.FC<MarketScoreCalculatorProps> = ({ onScoreCalculated }) => {
  ...
  const handleCalculate = async () => {
    ...
    try {
      const request: ScoreRequest = { features };
      const response = await recommendationsAPI.calculateScore(request);
      setResult(response);
      onScoreCalculated?.(response);
    } catch (err: any) {
      setError(err.response?.data?.error || "점수 계산에 실패했습니다.");
    } finally {
      setLoading(false);
    }
  };
            ...
            <div key={key} className={styles.featureItem}>
              ...
              <div className={styles.sliderContainer}>
                <input
                  ...
                  step="0.01"
                  value={value}
                  onChange={(e) => handleFeatureChange(key as keyof typeof features, parseFloat(e.target.value))}
          ...
          <div className={styles.scoreCard}>
            <div className={styles.scoreDisplay}>
              ...
              <div className={styles.scoreInfo}>
                <div className={styles.scoreLabel}>{getScoreLabel(result.score)}</div>
                ...
                <div key={key} className={styles.breakdownItem}>
                  ...
                  <div className={styles.breakdownDetails}>
                    <div className={styles.breakdownRow}>
                      <span>가중치:</span>
                      <span>{data.weight > 0 ? '+' : ''}{data.weight}</span>
                    </div>
                    <div className={styles.breakdownRow}>
                      <span>기여도:</span>
                      <span className={data.contrib >= 0 ? styles.positive : styles.negative}>
